...
const yamlFile = 'scoopDatabase.yml'
...
function saveDatabase () {
  writeYaml(yamlFile, database, function (err) {
    if (err) {
      console.log(err);
    }
  });
}  // end function saveDatabase

function loadDatabase () {
  let read = require('read-data');
  return read.sync(yamlFile);
...
function deleteComment (url, request) {
  ...
  if (savedComment) {
    ...
    // remove references to the comment in the user data
    const userCommentIds = database.users[savedComment.username].commentIds;
    userCommentIds.splice(userCommentIds.indexOf(id), 1);
    // remove references to the comment in the article data
  ...
  if (method === 'GET' || method === 'DELETE') {
    ...
    request.on('data', (chunk) => {
      ...
      if (typeof loadDatabase === 'function' && !isTestMode) {
        const savedDatabase2 = loadDatabase();
      }
